...
def addattendance():
    ...
    # Performing an inner join between Student and Attendance table based on "roll no"
    joined_data = (
        db.session.query(Student, Attendence)
        .filter(Student.rollno == Attendence.rollno)
        .all()
    )
